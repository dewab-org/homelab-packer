...
source "proxmox-iso" "windows_server_2022_desktop_experience" {
  proxmox_url   = local.proxmox_url
  username      = local.proxmox_user
  password      = local.proxmox_password
  node          = local.proxmox_node
  vm_id         = var.vm_id
  vm_name       = "windows-server-2022-desktop-experience-template"
  template_name = "windows-server-2022-desktop-experience"

  # Homelab-friendly: avoid TLS failures when the API endpoint hostname doesn't
  # match the certificate CN/SAN.
  insecure_skip_tls_verify = true

  template_description = "Windows Server 2022 Desktop Experience template generated by Packer on ${formatdate("YYYY-MM-DD hh:mm:ss 'UTC'", timestamp())}"
  onboot               = true
  os                   = "win10"
...
build {
  sources = ["source.proxmox-iso.windows_server_2022_desktop_experience"]
