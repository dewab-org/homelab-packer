...
- `../../common/scripts/bootstrap-base-template.py` creates or updates the base template from Canonical's cloud image. By default it uses VMID `9310` and template name `ubuntu-24-04-cloud-base`.
...
- Template customization reuses `../24.04-iso/ansible/configure.yml`, the same Ansible playbook used by the Ubuntu ISO/autoinstall build.
...
source .env
set +a
OVERWRITE_EXISTING=true builds/linux/common/scripts/bootstrap-base-template.py
```
