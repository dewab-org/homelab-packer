...
source "proxmox-iso" "rhel_9" {
  proxmox_url          = local.proxmox_url
  username             = local.proxmox_user
  password             = local.proxmox_password
  node                 = local.proxmox_node
  vm_id                = var.vm_id
  vm_name              = "rhel-9-template"
  template_name        = "rhel-9"
  template_description = "RHEL 9 template generated by Packer on ${formatdate("YYYY-MM-DD hh:mm:ss UTC", timestamp())}"
  onboot               = true
...
build {
  sources = ["source.proxmox-iso.rhel_9"]
