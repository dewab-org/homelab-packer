...
source "proxmox-iso" "rocky_8" {
  proxmox_url          = local.proxmox_url
  username             = local.proxmox_user
  password             = local.proxmox_password
  node                 = local.proxmox_node
  vm_id                = var.vm_id
  vm_name              = "rocky-8-template"
  template_name        = "rocky-8"
  template_description = "Rocky Linux 8 template generated by Packer on ${formatdate("YYYY-MM-DD hh:mm:ss UTC", timestamp())}"
  onboot               = true
...
build {
  sources = ["source.proxmox-iso.rocky_8"]
