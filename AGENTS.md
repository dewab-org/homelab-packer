...
- Usage: `./build.py builds/linux/rhel/10` (add `--ask`, `--overwrite`, or `--skip` as needed).
...
- RHEL builds register against Red Hat Network (RHN) using credentials from Vault (`secret/data/packer` for Packer `vault()`, `secret/packer` for Vault CLI access).
- RHEL/Rocky 8 uses Python 3.9 in the guest image for Ansible compatibility; keep ansible-core at 2.15.
